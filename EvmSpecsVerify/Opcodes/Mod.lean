import EvmSpecsVerify.Opcodes.Shapes.Binop

/-!
# MOD

Derived through `binop_step_equiv` (`Opcodes/Shapes/Binop.lean`); per-opcode
content is the pure-function lemma and the wf bound. Reachable outcomes:
success / stack underflow / out-of-gas (overflow unreachable for 2-in/1-out).
-/

set_option maxHeartbeats 1000000

namespace EvmSpecsVerify

open EvmAsm.Stateless.SpecRef
open Evm.Defs

theorem alu_mod_eq (a b : Nat) :
    Evm.Functions.alu_mod a b = (if b == 0 then 0 else a % b) := by
  show (if (b == 0) = true then ((0 : Int)).toNat else a % b)
      = if b == 0 then 0 else a % b
  split <;> rfl

theorem mod_wf (x y : Nat) (hx : WordWf x) :
    WordWf (if y == 0 then 0 else x % y) := by
  unfold WordWf at hx ⊢
  split
  · exact Nat.two_pow_pos 256
  · exact Nat.lt_of_le_of_lt (Nat.mod_le x _) hx

open Evm.Functions in
/-- **MOD, all reachable outcomes.** -/
theorem mod_step_equiv (sRef : Machine) (top : StackTop) (g : Nat)
    (hs : Evm.HostState) (ss : SeqState) (mem : EvmMemorySlice) (pc_in : Nat)
    (hrel : StateRel sRef top g hs ss)
    (hpc : pc_in = sRef.evm.pc + 1) :
    StepResultRel (AluPost mem) (runR iMod sRef)
      (runS (Evm.Functions.execute (.MOD ()) pc_in top mem g) hs ss) :=
  binop_step_equiv (.MOD ()) G_low alu_mod iMod GasCosts.OPCODE_MOD
    (fun x y => if y == 0 then 0 else x % y) rfl rfl ⟨rfl, fun _ _ _ _ => rfl⟩
    (fun x y _ _ => alu_mod_eq x y) (fun x y hx _ => mod_wf x y hx)
    sRef top g hs ss mem pc_in hrel hpc

end EvmSpecsVerify
