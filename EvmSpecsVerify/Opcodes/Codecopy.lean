...
The CALLDATACOPY sibling through SpecRef's shared `copyFromBuffer`: pop
destination / source / size, charge base + per-word copy + memory
expansion (the same three-stage extraction charge against SpecRef's
single `charge_gas`), extend, and splice the zero-padded **code** window
into memory. The source side goes through the `frame_code` register and
[`codeRel_copy`](../Relations/Code.lean) (both sides zero-pad past the
code end); code regions are immutable, so no `CalldataBelow`-style
separation hypothesis is needed. MM-6 (`MemGasSafe`) discharges the u32
`memory_access` guard. Reachable outcomes: success ×3 (zero size / grow /
in-window) / underflow ×3 / OOG at any of the three charges; overflow
unreachable (3-in/0-out).
-/
...
/-- The SpecRef CODECOPY charge for destination `x`, size `z` against
memory size `msz` (same shape as `cdcopyCost`; the bases agree at 3). -/
def codecopyCost (msz x z : Nat) : Nat :=
  GasCosts.OPCODE_CODECOPY_BASE
    + GasCosts.OPCODE_COPY_PER_WORD * (ceil32 z / 32)
    + (calculate_gas_extend_memory msz [(x, z)]).cost

/-! ## SpecRef run shapes -/

theorem runR_iCodecopy_underflow_nil (s : Machine)
    (hstack : s.evm.stack = []) :
    runR iCodecopy s = .ok (.error .stackUnderflow, s) := by
  simp only [iCodecopy, copyFromBuffer]
  exact runR_bind_err (runR_stackPop_nil s hstack)

theorem runR_iCodecopy_underflow_one (s : Machine) (x : U256)
...
theorem runR_iCodecopy_underflow_two (s : Machine) (x y : U256)
...
theorem runR_iCodecopy_oog (s : Machine) (x y z : U256)
    (rest : List U256)
    (hstack : s.evm.stack = x :: y :: z :: rest)
    (hgas : s.evm.gasLeft < codecopyCost s.evm.memory.length x z) :
    runR iCodecopy s =
      .ok (.error .outOfGas,
  ...
  exact runR_bind_err (runR_charge_gas_oog _ _
    (by simpa [codecopyCost] using hgas))

theorem runR_iCodecopy_success (s : Machine) (x y z : U256)
    (rest : List U256)
    (hstack : s.evm.stack = x :: y :: z :: rest)
    (hgas : codecopyCost s.evm.memory.length x z ≤ s.evm.gasLeft) :
    runR iCodecopy s =
      .ok (.ok (),
            ...
            memory := memory_write
              (s.evm.memory ++ List.replicate
                (calculate_gas_extend_memory s.evm.memory.length
                  [(x, z)]).expandBy 0x00) x
              (buffer_read s.evm.code y z)
            gasLeft := s.evm.gasLeft - codecopyCost s.evm.memory.length x z
            regularGasUsed :=
              s.evm.regularGasUsed + codecopyCost s.evm.memory.length x z
            pc := s.evm.pc + 1 } }) := by
  simp only [iCodecopy, copyFromBuffer, extendMemory, pcNext]
  refine runR_bind_ok (runR_stackPop_cons s x (y :: z :: rest) hstack) ?_
  refine runR_bind_ok (runR_stackPop_cons _ y (z :: rest) (by simp)) ?_
  refine runR_bind_ok (runR_stackPop_cons _ z rest (by simp)) ?_
  refine runR_bind_ok (runR_getEvm_map _ _) ?_
  refine runR_bind_ok (runR_charge_gas _ _
    (by simpa [codecopyCost] using hgas)) ?_
...
/-- The dispatch equation for CODECOPY. -/
theorem codecopy_dispatch (pc_in : Nat) (top : StackTop) (off len : Nat)
    (msf : EvmMemorySliceFields off len) (g : Nat) :
    Evm.Functions.execute_opcode (.CODECOPY ()) pc_in top
        ⟨off, len, msf⟩ g =
      Evm.Functions.execute_codecopy top ⟨off, len, msf⟩ g >>= fun p =>
        pure (pc_in, p.1, p.2.1, p.2.2) := rfl

open Evm.Functions in
theorem runS_execute_codecopy_underflow (pc_in : Nat) (top : StackTop)
...
theorem runS_execute_codecopy_oog_base (pc_in : Nat) (top : StackTop)
  ...
  rw [dif_pos rfl, codecopy_dispatch]
  have hbody : runS
      (Evm.Functions.execute_codecopy top ⟨off, len, msf⟩ g) hs ss =
      .ok ((top, (⟨off, len, msf⟩ : EvmMemorySlice), GAS_ZERO), hs)
        { ss with regs := haltRegs ss msg .OutOfGas } := by
    simp only [Evm.Functions.execute_codecopy]
    refine runS_bind_ok
...
open Evm.Functions in
theorem runS_execute_codecopy_oog_copy (pc_in : Nat) (top : StackTop)
    (off len g : Nat) (msf : EvmMemorySliceFields off len)
    (hs : Evm.HostState) (ss : SeqState)
...
open Evm.Functions in
theorem runS_execute_codecopy_oog_exp (pc_in : Nat) (top : StackTop)
    (off len g : Nat) (msf : EvmMemorySliceFields off len)
    (hs : Evm.HostState) (ss : SeqState)
    (l : List word) (frest : List (List word)) (x y z : Nat)
    (rest : List word)
...
theorem runS_execute_codecopy_ok_zero (pc_in : Nat) (top : StackTop)
    (off len g : Nat) (msf : EvmMemorySliceFields off len)
    (hs : Evm.HostState) (ss : SeqState)
    (l : List word) (frest : List (List word)) (x y : Nat)
    (rest : List word) (C : Bytes)
    ...
    (hcreg : ss.regs.get? Register.frame_code = some ⟨coff, clen, cf⟩)
    (hclen : clen = C.length)
    (hcbytes : ∀ i, i < clen → hs.codeBytes.getD (coff + i) 0 = C.getD i 0)
    (hbase : G_verylow ≤ g) :
    runS (Evm.Functions.execute (.CODECOPY ()) pc_in top
        ⟨off, len, msf⟩ g) hs ss =
      .ok ((pc_in, top - BitVec.ofNat 64 1 - BitVec.ofNat 64 1
  ...
  have hbody : runS
    ...
    refine runS_bind_ok (runS_readReg _ _ _ _ hcreg) ?_
    refine runS_bind_ok
      (codeRel_copy C hs ss coff clen (code_bytes cf) 0 y 0
        ({ base := off, established := len } : Evm.MemoryFrame) mfrest
        hmframe (by omega) hclen hcbytes) ?_
    rw [buffer_read_nil]
    exact runS_pure _ _ _
  exact runS_bind_ok hbody (runS_pure _ _ _)
...
zero-padded code window lands at `x`. The expansion zero-fill touches
only `memoryBytes`, so the code window agreement carries over
unchanged. -/
theorem runS_execute_codecopy_ok_grow (pc_in : Nat) (top : StackTop)
    (off len g : Nat) (msf : EvmMemorySliceFields off len)
    (hs : Evm.HostState) (ss : SeqState)
    (l : List word) (frest : List (List word)) (x y z : Nat)
    (rest : List word) (C : Bytes)
  ...
  have hbody : runS
    ...
    refine runS_bind_ok (runS_readReg _ _ _ _ hcreg) ?_
    refine runS_bind_ok
      (codeRel_copy C _ ss coff clen (code_bytes cf) x y z
        ({ base := off, established := x + z } : Evm.MemoryFrame) mfrest
        rfl (le_refl _) hclen hcbytes) ?_
    exact runS_pure _ _ _
  exact runS_bind_ok hbody (runS_pure _ _ _)

open Evm.Functions in
/-- Success inside the established window: no expansion, the code window
lands at `x`. -/
theorem runS_execute_codecopy_ok_nogrow (pc_in : Nat) (top : StackTop)
    (off len g : Nat) (msf : EvmMemorySliceFields off len)
    (hs : Evm.HostState) (ss : SeqState)
    (l : List word) (frest : List (List word)) (x y z : Nat)
    (rest : List word) (C : Bytes)
  ...
  have hbody : runS
    ...
    refine runS_bind_ok (runS_readReg _ _ _ _ hcreg) ?_
    refine runS_bind_ok
      (codeRel_copy C hs ss coff clen (code_bytes cf) x y z
        ({ base := off, established := len } : Evm.MemoryFrame) mfrest
        hmframe (by omega) hclen hcbytes) ?_
    exact runS_pure _ _ _
  exact runS_bind_ok hbody (runS_pure _ _ _)
...
open Evm.Functions in
/-- **CODECOPY, all reachable outcomes** (success splits on zero size and
on whether the frame window grows). Requires the memory relation, the
MM-6 gas budget, and the code relation (`frame_code` register window ↔
SpecRef's `evm.code`). -/
theorem codecopy_step_equiv (sRef : Machine) (top : StackTop) (g : Nat)
    ...
    StepResultRel MemPost (runR iCodecopy sRef)
      (runS (Evm.Functions.execute (.CODECOPY ()) pc_in top
  ...
  obtain ⟨coff, clen, cf, hcreg, hclen, hcbytes⟩ := hcrel
  match hS : sRef.evm.stack with
  | [] =>
    rw [hS] at hpfx htop
    rw [runR_iCodecopy_underflow_nil sRef hS,
      runS_execute_codecopy_underflow pc_in top off len g msf hs ss prof
  ...
  | [x] =>
    rw [hS] at hpfx htop
    rw [runR_iCodecopy_underflow_one sRef x hS,
      runS_execute_codecopy_underflow pc_in top off len g msf hs ss prof
        sRef.evm.stateGasSpilled msg hprof hsp hmsg hfork
        (by simp at htop; omega)]
    exact StepResultRel.halted ErrorRel.stackUnderflow
  ...
  | [x, y] =>
    rw [hS] at hpfx htop
    rw [runR_iCodecopy_underflow_two sRef x y hS,
      runS_execute_codecopy_underflow pc_in top off len g msf hs ss prof
        sRef.evm.stateGasSpilled msg hprof hsp hmsg hfork
        (by simp at htop; omega)]
    exact StepResultRel.halted ErrorRel.stackUnderflow
  ...
  | x :: y :: z :: rest =>
    ...
    have hTsplit : codecopyCost sRef.evm.memory.length x z
        = 3 + 3 * Evm.Functions.memory_word_count z
          + Evm.Functions.memory_expansion_cost ⟨off, len, msf⟩
              (Evm.Functions.memory_required_size x z) := by
      rw [codecopyCost, hwcz, ← hcost]
      rfl
    by_cases hg : sRef.evm.gasLeft < codecopyCost sRef.evm.memory.length x z
    · rw [runR_iCodecopy_oog sRef x y z rest hS hg]
      ...
      · rw [runS_execute_codecopy_oog_base pc_in top off len g msf hs ss
      ...
      · push Not at hb
        ...
        by_cases hc : g - G_verylow
            < G_copy_word * Evm.Functions.memory_word_count z
        · rw [runS_execute_codecopy_oog_copy pc_in top off len g msf hs
        ...
        · push Not at hc
          have hcN : (3 : Nat) * Evm.Functions.memory_word_count z
              ≤ g - 3 := hc
          rw [runS_execute_codecopy_oog_exp pc_in top off len g msf hs
            ss l frest x y z rest hframe hpfx htop hlim' prof
            sRef.evm.stateGasSpilled msg hprof hsp hmsg hfork hzw hb hc
            (by
    ...
    · push Not at hg
      have hgT : 3 + 3 * Evm.Functions.memory_word_count z
        ...
        rw [← hTsplit]
        show codecopyCost sRef.evm.memory.length x z ≤ g
        rw [hlive]
        exact hg
      ...
      have hgas' : g - 3 - 3 * Evm.Functions.memory_word_count z
          ...
          = (sRef.evm.gasLeft : Nat)
            - codecopyCost sRef.evm.memory.length x z := by
        rw [hTsplit, ← hlive]
        omega
      rw [runR_iCodecopy_success sRef x y z rest hS hg]
      by_cases hz0 : z = 0
      · subst hz0
        have hzero : (calculate_gas_extend_memory sRef.evm.memory.length
          ...
          rfl
        rw [runS_execute_codecopy_ok_zero pc_in top off len g msf hs ss
          l frest x y rest sRef.evm.code coff clen cf mfrest hframe hpfx
          htop hlim' hmframe hcreg hclen hcbytes hbase]
        ...
        have hcost0 : codecopyCost sRef.evm.memory.length x 0 = 3 := by
          rw [codecopyCost, hzero]
          rfl
        ...
        · refine ⟨?_, hres, hsp⟩
          show g - 3
            = sRef.evm.gasLeft - codecopyCost sRef.evm.memory.length x 0
          rw [hcost0, hlive]
      ...
      · have hz : (z == 0) = false := by simpa using hz0
        ...
        have hvlen : (buffer_read sRef.evm.code y z).length = z :=
          buffer_read_length _ _ _
        by_cases hgrow : len < x + z
        · rw [runS_execute_codecopy_ok_grow pc_in top off len g msf hs
            ss l frest x y z rest sRef.evm.code coff clen cf mfrest hframe
            hpfx htop hlim' hmframe hcreg hclen hcbytes hz hzw hbase hcopy
            hexp hreq hgrow]
          ...
          have hrelStored := memoryRel_write _ _ off (x + z) x
            (buffer_read sRef.evm.code y z) hrel'
            (by rw [hvlen])
          rw [hexpandBy]
          refine StepResultRel.success ?_
          ...
          · refine ⟨off, x + z, {}, rfl, ?_, ?_⟩
            ...
            · show MemGasSafe _ (sRef.evm.gasLeft - codecopyCost
              ...
              exact hsafe2
        · push Not at hgrow
          rw [runS_execute_codecopy_ok_nogrow pc_in top off len g msf
            hs ss l frest x y z rest sRef.evm.code coff clen cf mfrest
            hframe hpfx htop hlim' hmframe hcreg hclen hcbytes hz hzw hbase
            hcopy hexp hreq hgrow]
          have hrelStored := memoryRel_write sRef.evm.memory hs off len x
            (buffer_read sRef.evm.code y z)
          ...
          · refine ⟨off, len, msf, rfl, ?_, ?_⟩
            ...
            · show MemGasSafe _ (sRef.evm.gasLeft - codecopyCost
              ...
              have hsafe2 := memGasSafe_mono_gas sRef.evm.memory
                (g' := sRef.evm.gasLeft
                  - codecopyCost sRef.evm.memory.length x z)
