...
[ADDRESS](Address.lean) with the caller field: charge
`G_base`/`OPCODE_CALLER` (= 2, both sides), push the calling account's
address as a word (`address_to_word` ↔ `bytesBEtoNat`, identified by
`address_to_word_eq` under the `hcaller` register-tie hypothesis).
...
theorem runR_iCaller_success (s : Machine)
    (hlen : s.evm.stack.length ≠ 1024)
    (hgas : GasCosts.OPCODE_CALLER ≤ s.evm.gasLeft) :
    runR iCaller s =
      .ok (.ok (),
        { s with evm := { s.evm with
            stack := bytesBEtoNat s.evm.message.caller :: s.evm.stack
            gasLeft := s.evm.gasLeft - GasCosts.OPCODE_CALLER
            regularGasUsed :=
              s.evm.regularGasUsed + GasCosts.OPCODE_CALLER
            pc := s.evm.pc + 1 } }) := by
  simp only [iCaller, pcNext]
...
theorem runR_iCaller_overflow (s : Machine)
    (hlen : s.evm.stack.length = 1024)
    (hgas : GasCosts.OPCODE_CALLER ≤ s.evm.gasLeft) :
    runR iCaller s =
      .ok (.error .stackOverflow,
        { s with evm := { s.evm with
            gasLeft := s.evm.gasLeft - GasCosts.OPCODE_CALLER
            regularGasUsed :=
              s.evm.regularGasUsed + GasCosts.OPCODE_CALLER } }) := by
  simp only [iCaller]
  refine runR_bind_ok (runR_charge_gas _ _ hgas) ?_
  refine runR_bind_ok (runR_getEvm _) ?_
...
theorem runR_iCaller_oog (s : Machine)
    (hgas : s.evm.gasLeft < GasCosts.OPCODE_CALLER) :
    runR iCaller s = .ok (.error .outOfGas, s) := by
  simp only [iCaller]
...
/-- The dispatch equation for CALLER. -/
theorem caller_dispatch (pc_in : Nat) (top : StackTop)
    (mem : EvmMemorySlice) (g : Nat) :
    Evm.Functions.execute_opcode (.CALLER ()) pc_in top mem g =
      Evm.Functions.execute_caller top g >>= fun p =>
        pure (pc_in, p.1, mem, p.2) := rfl

open Evm.Functions in
theorem runS_caller_body_ok (top : StackTop) (g : Nat)
    ...
    (hgas : (GasCosts.OPCODE_CALLER : Nat) ≤ g) :
    runS (Evm.Functions.execute_caller top g) hs ss =
      .ok ((top + BitVec.ofNat 64 1, g - GasCosts.OPCODE_CALLER),
        { hs with stackFrames :=
            writeListAt l top.toNat (address_to_word msg.caller) :: frest })
        ss := by
  simp only [Evm.Functions.execute_caller]
  ...
  refine runS_bind_ok
    (runS_push_word top (address_to_word msg.caller) hs ss l frest hframe
      hbound) ?_
  exact runS_pure _ _ _

open Evm.Functions in
theorem runS_caller_body_oog (top : StackTop) (g : Nat)
    (hs : Evm.HostState) (ss : SeqState)
    ...
    (hgas : g < (GasCosts.OPCODE_CALLER : Nat)) :
    runS (Evm.Functions.execute_caller top g) hs ss =
      .ok ((top, GAS_ZERO), hs)
...
open Evm.Functions in
theorem runS_execute_caller_success (pc_in : Nat) (top : StackTop) (g : Nat)
    ...
    (hbound : top.toNat + 1 < 2 ^ 64)
    (hgas : (GasCosts.OPCODE_CALLER : Nat) ≤ g) :
    runS (Evm.Functions.execute (.CALLER ()) pc_in top mem g) hs ss =
      .ok ((pc_in, top + BitVec.ofNat 64 1, mem,
          g - GasCosts.OPCODE_CALLER),
        { hs with stackFrames :=
            writeListAt l top.toNat (address_to_word msg.caller) :: frest })
        ss := by
  simp only [Evm.Functions.execute,
    show Evm.Functions.opcode_stack_effect (.CALLER ()) = pure (0, 1)
      from rfl]
  refine runS_bind_ok (runS_pure _ _ _) ?_
  refine runS_bind_ok
  ...
  rw [dif_pos rfl, caller_dispatch]
  refine runS_bind_ok
    (runS_caller_body_ok top g hs ss l frest msg hframe hmsg hbound
      hgas) ?_
  exact runS_pure _ _ _

open Evm.Functions in
theorem runS_execute_caller_overflow (pc_in : Nat) (top : StackTop)
...
open Evm.Functions in
theorem runS_execute_caller_oog (pc_in : Nat) (top : StackTop) (g : Nat)
    (mem : EvmMemorySlice) (hs : Evm.HostState) (ss : SeqState)
    (prof : ExecutionProfile) (sp : state_gas_spill) (msg : Evm.Defs.Message)
    (hprof : ss.regs.get? Register.k_execution_profile = some prof)
    (hsp : ss.regs.get? Register.state_gas_spilled = some sp)
  ...
  rw [dif_pos rfl, caller_dispatch]
  refine runS_bind_ok
    (runS_caller_body_oog top g hs ss prof sp msg hprof hsp hmsg hfork
      hgas) ?_
  exact runS_pure _ _ _
...
open Evm.Functions in
/-- **CALLER, all reachable outcomes.** Double-fault states (full stack ∧
OOG) use the MM-5 constructor; `hcaller` ties the message register to
SpecRef's calling account. -/
theorem caller_step_equiv (sRef : Machine) (top : StackTop) (g : Nat)
  ...
  have hax : msg.caller.toList = sRef.evm.message.caller :=
    hcaller msg hmsg
  by_cases hg : sRef.evm.gasLeft < GasCosts.OPCODE_CALLER
  · rw [runR_iCaller_oog sRef hg]
    by_cases hov : sRef.evm.stack.length = 1024
    · rw [runS_execute_caller_overflow pc_in top g mem hs ss prof
    ...
    · rw [runS_execute_caller_oog pc_in top g mem hs ss prof
        sRef.evm.stateGasSpilled msg hprof hsp hmsg hfork (by omega)
  ...
  · push Not at hg
    by_cases hov : sRef.evm.stack.length = 1024
    · rw [runR_iCaller_overflow sRef hov hg,
        runS_execute_caller_overflow pc_in top g mem hs ss prof
          sRef.evm.stateGasSpilled msg hprof hsp hmsg hfork (by omega)]
    ...
    · have hbound : top.toNat + 1 < 2 ^ 64 := by
        ...
        omega
      rw [runR_iCaller_success sRef hov hg,
        runS_execute_caller_success pc_in top g mem hs ss l frest msg
      ...
      · refine ⟨⟨writeListAt l top.toNat
            (bytesBEtoNat sRef.evm.message.caller), frest, rfl,
            ?_, ?_⟩, ?_, ?_, ?_⟩
        ...
        · intro w hw
          ...
          · subst hw
            ...
            exact address_to_word_wf msg.caller
          · exact hwfS w hw
