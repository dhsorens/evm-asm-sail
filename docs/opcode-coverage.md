...
[`EvmSpecsVerify/Coverage/Registry.lean`](../EvmSpecsVerify/Coverage/Registry.lean)
makes the same classification machine-checked against the AST itself:
[`astStatus`](../EvmSpecsVerify/Coverage/Registry.lean#L160) is **total** over
`Evm.Defs.ast`, so a constructor with no row fails to compile rather than going
unnoticed, and the counts are computed from a verified enumeration
([`astStatusCount`](../EvmSpecsVerify/Coverage/Registry.lean#L369)) instead of
...
**Keep a row's whole status note inside one pair of parentheses** — the parser
reads the theorem link only out of a parenthesised body that runs to the end of
the cell, so a cell that closes early and keeps writing keeps its status but
loses its theorem name, line and deep link from the generated site.
`check_row_theorems` now fails the refresh when that happens.
...
| opcode | byte | SpecRef | `Evm` | shape | status |
|---|---|---|---|---|---|
| ADDRESS | 0x30 | `iAddress` | `execute_address` | env | **full** ([`address_step_equiv`](../EvmSpecsVerify/Opcodes/Address.lean#L211) — success/overflow/OOG/MM-5 double fault; codec bridge `address_to_word_eq`) |
| BALANCE | 0x31 | `iBalance` | `execute_balance` | env+world | **full** ([`balance_step_equiv`](../EvmSpecsVerify/Opcodes/Balance.lean#L457) — success (warm/cold)/underflow/OOG ×2; `WarmAddrRel` (prewarm invariant + classifier shape hypotheses), value behind ledgered `BalanceAgree`, now dischargeable for an account already written in this transaction by `balanceAgree_of_accountRel` (`AccountRel`, Relations/Account.lean) — see `Assumptions.lean`) |
...
| CALLDATALOAD | 0x35 | `iCalldataload` | `execute_calldataload` | env+memory | **full** ([`calldataload_step_equiv`](../EvmSpecsVerify/Opcodes/Calldataload.lean#L223) — success/underflow/OOG; `CalldataRel` covers both calldata windows (input arena / parent memory), both sides zero-pad so no range hypothesis) |
...
| EXTCODEHASH | 0x3f | `iExtcodehash` | `execute_extcodehash` | env+world+crypto | **full** ([`extcodehash_step_equiv`](../EvmSpecsVerify/Opcodes/Extcodehash.lean#L544) — warm/cold success and OOG plus underflow; `WarmAddrRel` proves account-access classification/gas, while ledgered `ExtcodehashAgree` ties missing-account zero and the 32-byte code-hash codec — now dischargeable on the transaction-overlay hit by `extcodehashAgree_of_accountRel` (`AccountRel`), which also machine-checks `EMPTY_CODE_HASH = KECCAK_EMPTY` (`empty_code_hash_eq`)) |
...
| SELFBALANCE | 0x47 | `iSelfbalance` | `execute_selfbalance` | env+world | **full** ([`selfbalance_step_equiv`](../EvmSpecsVerify/Opcodes/Selfbalance.lean#L271) — success/overflow/OOG/MM-5 double fault; underflow unreachable for 0-in. No warm/cold split: the own account is a flat `5` on both sides, so `SelfBalanceAgree` needs no warm-stamp quantification unlike `BalanceAgree`; dischargeable on the transaction-overlay hit by `selfBalanceAgree_of_accountRel` (`AccountRel`) — see `Assumptions.lean`) |
...
| MLOAD | 0x51 | `iMload` | `execute_mload` | memory | **full** ([`mload_step_equiv`](../EvmSpecsVerify/Opcodes/Mload.lean#L392) — success (grow/in-window)/underflow/OOG ×2; `MemoryRel` + MM-6 `MemGasSafe` hypotheses) |
...
| TLOAD | 0x5c | `iTload` | `execute_tload` | storage (transient) | **full** ([`tload_step_equiv`](../EvmSpecsVerify/Opcodes/Tload.lean#L263) — success/underflow/OOG; overflow unreachable for 1-in/1-out. EIP-1153 prices transient access flat on both sides, so `TloadAgree` needs no warm-stamp quantification unlike `SloadAgree`; the read value was behind a ledgered hypothesis until TSTORE landed `TransientRel`: `tloadAgree_of_transientRel` now proves `TloadAgree` outright, so nothing here is assumed about transient storage any more) |
| TSTORE | 0x5d | `iTstore` | `execute_tstore` | storage (transient) | **full** ([`tstore_step_equiv`](../EvmSpecsVerify/Opcodes/Tstore.lean#L293) — success, the static halt, underflow ×2, OOG, plus the MM-14 double fault; overflow unreachable for 2-in/0-out. The first opcode whose whole observable effect is a world-ish write, so its post is [`TransientPost`](../EvmSpecsVerify/Relations/Transient.lean) = `BasePost` ∧ `TransientRel`, with `transientRel_write` proving the store relation survives the write. MM-13 records why that relation is pointwise: SpecRef deletes the row on a zero write where the extraction stores the zero. MM-14 is the mirror of MM-11 — SpecRef's static guard precedes its pops, the extraction's follows the hoisted `validate_stack`) |
| MCOPY | 0x5e | `iMcopy` | `execute_mcopy` | memory | **full** ([`mcopy_step_equiv`](../EvmSpecsVerify/Opcodes/Mcopy.lean#L702) — success ×3 (zero length/grow/in-window), underflow ×3, OOG at each of the extraction's three charges; overflow unreachable. Two new pieces: `calc_extend_pair_eq_single` proves SpecRef's two-range extension fold telescopes to the single range at `max destination source`, and `memoryRel_mcopy` (via the new `memoryRel_read`) proves the overlapping-safe copy agrees — both sides snapshot the source before writing, so no disjointness hypothesis is needed. `MemoryRel` + MM-6 `MemGasSafe`) |
...
| SWAP n | 0x90–0x9f | `iSwapN` | `execute_swap` | stack | **full** ([`swap_step_equiv`](../EvmSpecsVerify/Opcodes/Swap.lean#L371) — success/underflow/OOG/MM-5 double fault; overflow unreachable since the height is unchanged. `take_swap_writes` bridges SpecRef's head-indexed `listSwap S 0 n` to the extraction's two `stack_set` writes at cursor slots 0 and n; index conventions already agree (`iSwapN (op - 0x8F)`)) |
| LOG n | 0xa0–0xa4 | `iLogN` | `execute_log` | memory+logs | **full** ([`logn_step_equiv`](../EvmSpecsVerify/Opcodes/Log.lean#L829) — one theorem for all five arities: success ×3 (zero size/grow/in-window), underflow at any height below `n + 2`, the static halt, OOG at each live charge stage (base, topics, payload, expansion), plus the MM-11 double fault. Targets [`LogPost`](../EvmSpecsVerify/Relations/Log.lean) = `MemPost` ∧ `LogRel`. The arity is not a case split: `runR_mapM_topics_ok` handles SpecRef's `(List.range n).mapM` by induction, `runS_pop_log_topics` the extraction's five-way match, and `topicWords_logTopicsOf` collapses the `LogTopics` constructor back to a list. `hn : n ≤ 4` is the decoders' own range — MM-12 records what the extraction's catch-all does past it) |
...
| RETURN | 0xf3 | `iReturn` | `execute_return` | system | **full** ([`return_step_equiv`](../EvmSpecsVerify/Opcodes/Return.lean#L561) — normal halt with output correspondence (`ReturnPost`), zero/grow/in-window reads, underflow ×2, expansion OOG) |
...
| REVERT | 0xfd | `iRevert` | `execute_revert` | system | **full** ([`revert_step_equiv`](../EvmSpecsVerify/Opcodes/Revert.lean#L521) — revert with output correspondence (`RevertPost`), zero/grow/in-window reads, underflow ×2, expansion OOG. Targets [`RevertResultRel`](../EvmSpecsVerify/Relations/Outcome.lean#L150), not `StepResultRel`: `.revert` is not an exceptional halt, so the gas survives. New MM-9 records the state-gas refill's layer, proven equal to SpecRef's own `refill_frame_state_gas` via `runR_refill`) |
| INVALID | 0xfe | (dispatch throws `.invalidOpcode`) | `execute_invalid` | system | **full** ([`invalid_step_equiv`](../EvmSpecsVerify/Opcodes/Invalid.lean#L155) — one reachable outcome, the `InvalidOpcode` halt with all gas consumed, through the plain `halted` constructor and a new [`ErrorRel`](../EvmSpecsVerify/Relations/Outcome.lean#L35) arm. Neither side has a named handler: the theorem's SpecRef subject is the catch-all's `throw (.invalidOpcode op)` (Interpreter.lean:360), written inline because there is no `def` to name — the same MM-3 exposure every other row carries, not a larger one, since `opImplementation … 0xfe = throw …` is exactly as unprovable as `… 0x01 = iAdd`. **Provably outside the double-fault class** (MM-5/MM-10/MM-11/MM-14): `opcode_stack_effect (.INVALID ()) = pure (0, 0)`, so `execute`'s hoisted `validate_stack` cannot fire under the 1024 invariant, and SpecRef reads nothing and charges nothing before throwing — the one opcode where that class is empty by proof rather than by case split. MM-4 is vacuous: neither side moves the pc. `Post` is universally quantified, the success branch being unreachable. The theorem also quantifies the byte, so it covers the whole undefined range: the extraction's decoder sends every byte with no `ast` constructor to this one node (Interpreter.lean:241), and SpecRef's catch-all is uniform in `op` outside the PUSH/DUP/SWAP ranges. No new assumptions — `StateRel` is the only hypothesis) |
| SELFDESTRUCT | 0xff | `iSelfdestruct` | `execute_selfdestruct` | system+world | **full** ([`selfdestruct_step_equiv`](../EvmSpecsVerify/Opcodes/Selfdestruct.lean#L2116) — success, static, underflow, sentry-OOG, execution-OOG and state-gas-OOG, six outcomes through one dispatcher. The Amsterdam schedule is proven outright (`selfdestructCost`, `newAccount_eq`, and `sdChargedEvm_gas` matching both gas dimensions against the extraction's), warm/cold accounting against `WarmAddrRel` (`warm_of_warmAddrRel`), the `creates_account` predicate against `AccountRel` (`sdCreates_eq`), the value transfer and its EIP-7708 log by [`transfer_equiv`](../EvmSpecsVerify/Relations/Transfer.lean) and its three degenerate siblings, and the EIP-6780 mark by [`LifecycleRel`](../EvmSpecsVerify/Relations/Selfdestruct.lean). **Closes MM-14**: the underflow outcome pairs SpecRef's `.writeInStaticContext` with the extraction's `StackUnderflow` through `haltedStaticFirst`, since `execute` hoists `validate_stack` above `guard_static` — SELFDESTRUCT was the last member of that class. **Closes MM-2's account-write subset.** The reads and the transfer's case sit behind the ledgered `SelfdestructAgree`, which is *reduced* in all four transfer shapes (`selfdestructTransfer_nondegenerate`/`_self`/`_zero`/`_zero_collapse`), so what stays assumed is which shape a state is in plus those pairings' own ledger conditions (MM-19's non-wrap bound, MM-18's collapse tests) and the two account rows being in the transaction overlay — the same domain restriction `BalanceAgree` carries. `SelfdestructAgree` also threads the EIP-7825 spill cap, an extraction-only hard abort with no SpecRef counterpart, exactly as `sstore_step_equiv` threads its own. MM-20 records why the EIP-6780 test costs a `CreatedAgree` hypothesis) |
