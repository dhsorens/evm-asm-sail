...
| logs | `Evm.logs : List Log` (Vm.lean:195) | `HostState.logs : Array LogRecordRow` + `logBytes` arena | [`LogRel`](../EvmSpecsVerify/Relations/Log.lean) (suffix from a frame `base`; address via `Vector.toList`, topics via `toBeBytes32`, payload via the arena span) | rows' spans lie inside the arena (`bounded`) | stated (`logRel_append` + `runS_k_log_memory` proven; no opcode consumes it yet — LOG is the next slice) |
| refund counter | `Evm.refundCounter : Int` (Vm.lean:196) | `frame_refund` register (`gas_refund := Int`) | [`RefundRel`](../EvmSpecsVerify/Relations/Refund.lean) (plain equality — both sides settle against the transaction's gas at tx end, not per step) | the extraction's `record_refund` validates the running sum against `±gas_refund_bound` and spec-aborts outside it (MM-17), threaded as a step hypothesis rather than a field | proven-sstore (`sstore_step_equiv` preserves it; `specRefundDelta` = the extraction's `amsterdam_sstore_costs.refund` by `sstore_refund_eq`) |
...
| component | SpecRef repr | `Evm` repr | relation | invariants | status |
|---|---|---|---|---|---|
| accounts | `BlockState`/`TransactionState` journals (StateTracker.lean:87,107) | `HostState.accountTx/accountBlock : List (address × AcctValue)` (+ `Evm.Contracts.ReferenceWorldState` spec layer, unconnected) | [`AccountRel`](../EvmSpecsVerify/Relations/Account.lean) (transaction overlay, pointwise and one-directional like `StorageRel`: SpecRef's entry is the extraction row's EIP-161 view — the three trie fields, or `none` when the row is not `present`) | existence is a row flag on one side and the shape of the entry on the other, so the relation carries the host's EIP-161 discipline (`absentEmpty`/`presentNonEmpty`); only the transaction layer is related — the block overlay doubles as the extraction's witness read-through cache and its miss path is an authenticated trie walk. `HostState.accountTx` also carries `storage_root` and the `created`/`selfdestructed` flags, which SpecRef keeps elsewhere (`createdAccounts`, `accountsToDelete`) | related (`balanceAgree_of_accountRel` / `selfBalanceAgree_of_accountRel` / `extcodehashAgree_of_accountRel` discharge the three account-read hypotheses on the transaction-overlay hit — the miss regimes stay ledgered; EXTCODESIZE/EXTCODECOPY read the code store, not this overlay) |
| persistent storage | `TransactionState.storageWrites` (tx) / `parent.storageWrites` (block) / witness pre-state | `HostState.storageTx/storageBlock : List (StorageKey × StorageValue)` | [`StorageRel`](../EvmSpecsVerify/Relations/Storage.lean) (transaction overlay, pointwise and **one-directional**: every extraction row is a SpecRef row with the same live value, not conversely — MM-16; the extraction's stored `orig` tied to SpecRef's `getStorageOriginal` via `specOrig`) | the block overlay doubles as the extraction's witness read-through cache and its miss path is an authenticated trie walk, so only the transaction layer is related; `HostState.storageCleared` pairs with SpecRef's `destroyStorage`/`createdAccounts` (CREATE family) | proven-sstore (`sstore_step_equiv` consumes and re-establishes it: `storageRel_write` for a changing write, `storageRel_write_noop` for the MM-16 no-op; `sloadAgree_of_storageRel`/`sstoreAgree_of_storageRel` discharge the read agreements on the transaction-overlay hit — the miss regimes stay ledgered) |
