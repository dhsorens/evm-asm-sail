...
| LOG n | 0xa0–0xa4 | `iLogN` | `execute_log` | memory+logs | **full** ([`logn_step_equiv`](../EvmSpecsVerify/Opcodes/Log.lean#L862) — one theorem for all five arities: success ×3 (zero size/grow/in-window), underflow at any height below `n + 2`, the static halt, OOG at each live charge stage (base, topics, payload, expansion), plus the MM-11 double fault. Targets [`LogPost`](../EvmSpecsVerify/Relations/Log.lean) = `MemPost` ∧ `LogRel`. The arity is not a case split: `runR_mapM_topics_ok` handles SpecRef's `(List.range n).mapM` by induction, `runS_pop_log_topics` the extraction's five-way match, and `topicWords_logTopicsOf` collapses the `LogTopics` constructor back to a list. `hn : n ≤ 4` is the decoders' own range — MM-12 records what the extraction's catch-all does past it) |
