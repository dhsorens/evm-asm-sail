...
| SELFDESTRUCT | 0xff | `iSelfdestruct` | `execute_selfdestruct` | system+world | unstated (every prerequisite has now landed: [`AccountRel`](../EvmSpecsVerify/Relations/Account.lean) for the overlay, [`transfer_equiv`](../EvmSpecsVerify/Relations/Transfer.lean) for the balance transfer + EIP-7708 log, and [`Relations/Selfdestruct.lean`](../EvmSpecsVerify/Relations/Selfdestruct.lean) for the Amsterdam schedule (`selfdestructCost`, closing MM-2's account-write subset), the `creates_account` predicate and the EIP-6780 lifecycle flags (`LifecycleRel`). Neither side deletes at the opcode — both defer to transaction end — so no collapsing write is involved, and `transfer_equiv_self`/`_zero`/`_zero_collapse` cover the two degenerate transfers a `SELFDESTRUCT` can reach (MM-18, now closed by proof). **SpecRef's side is complete**: all six outcomes are proven ([`Opcodes/Selfdestruct.lean`](../EvmSpecsVerify/Opcodes/Selfdestruct.lean): [`runR_iSelfdestruct_static`](../EvmSpecsVerify/Opcodes/Selfdestruct.lean), `_underflow`, `_sentry_oog`, `_charge_oog`, `_state_oog`, `_success`), factored through the shared prefix `runR_iSelfdestruct_prefix` (static test, pop, warm test, sentry, cold marking, and both reads) and the tail `sdChargeTail`. What is left is the extraction's side (a `SailME` early-return chain) and the composition, including the `running := false` ↔ `Halted HaltSelfDestruct` pairing (the STOP/RETURN normal-halt shape). The read audit against `execute_selfdestruct`'s Amsterdam branch is in the module docstring: the two sides read the same three facts in a different order and SpecRef reads the originator's account twice, none of it observable, but `runR_iSelfdestruct_success` carries the second read as a separate parameter rather than assuming it away. MM-14 covers its guard ordering; MM-20 records why the EIP-6780 test costs a `CreatedAgree` hypothesis) |
