...
`iMstore` pops offset and value, charges base + expansion once, extends by
the ceil32 block, and splices the 32-byte big-endian encoding; the
extraction charges base before the pops, establishes the exact byte mark,
and writes through the host codec. `memoryRel_store` ties `memory_write` of
`toBeBytes32` to `writeArrayBytes` of `wordBytes` (the codecs agree by
`wordBytes_eq`). Same charge-split equivalence, MM-6 budget discharge, and
grow/in-window split as [`Mload`](Mload.lean). Reachable outcomes: success
×2 / underflow ×2 / OOG at either charge; overflow unreachable (2-in/0-out).
-/

open private pcAdd chargeWithMemory from EvmAsm.Stateless.SpecRef.InstructionsCore
open private writeArrayBytes wordBytes zeroMemoryRange from Evm.HostAxioms
...
/-- The SpecRef MSTORE charge for offset `x` against memory size `msz`. -/
def mstoreCost (msz x : Nat) : Nat :=
  GasCosts.OPCODE_MSTORE_BASE
    + (calculate_gas_extend_memory msz [(x, 32)]).cost

/-! ## SpecRef run shapes -/

theorem runR_iMstore_underflow_nil (s : Machine)
    (hstack : s.evm.stack = []) :
    runR iMstore s = .ok (.error .stackUnderflow, s) := by
  simp only [iMstore]
  exact runR_bind_err (runR_stackPop_nil s hstack)

theorem runR_iMstore_underflow_one (s : Machine) (x : U256)
...
theorem runR_iMstore_oog (s : Machine) (x y : U256) (rest : List U256)
    (hstack : s.evm.stack = x :: y :: rest)
    (hgas : s.evm.gasLeft < mstoreCost s.evm.memory.length x) :
    runR iMstore s =
      .ok (.error .outOfGas,
        { s with evm := { s.evm with stack := rest } }) := by
  simp only [iMstore, chargeWithMemory]
  ...
  exact runR_bind_err (runR_bind_ok (runR_getEvm_map _ _)
    (runR_bind_err (runR_charge_gas_oog _ _
      (by simpa [mstoreCost] using hgas))))

theorem runR_iMstore_success (s : Machine) (x y : U256) (rest : List U256)
    (hstack : s.evm.stack = x :: y :: rest)
    (hgas : mstoreCost s.evm.memory.length x ≤ s.evm.gasLeft) :
    runR iMstore s =
      .ok (.ok (),
            ...
            memory := memory_write
              (s.evm.memory ++ List.replicate
                (calculate_gas_extend_memory s.evm.memory.length
                  [(x, 32)]).expandBy 0x00) x (toBeBytes32 y)
            gasLeft := s.evm.gasLeft - mstoreCost s.evm.memory.length x
            regularGasUsed :=
              s.evm.regularGasUsed + mstoreCost s.evm.memory.length x
            pc := s.evm.pc + 1 } }) := by
  simp only [iMstore, chargeWithMemory, extendMemory, pcAdd]
  refine runR_bind_ok (runR_stackPop_cons s x (y :: rest) hstack) ?_
  refine runR_bind_ok (runR_stackPop_cons _ y rest (by simp)) ?_
  refine runR_bind_ok (runR_bind_ok (runR_getEvm_map _ _)
    (runR_bind_ok (runR_charge_gas _ _ (by simpa [mstoreCost] using hgas))
...
/-- The dispatch equation for MSTORE. -/
theorem mstore_dispatch (pc_in : Nat) (top : StackTop) (off len : Nat)
    (msf : EvmMemorySliceFields off len) (g : Nat) :
    Evm.Functions.execute_opcode (.MSTORE ()) pc_in top ⟨off, len, msf⟩ g =
      Evm.Functions.execute_mstore top ⟨off, len, msf⟩ g >>= fun p =>
        pure (pc_in, p.1, p.2.1, p.2.2) := rfl

open Evm.Functions in
theorem runS_execute_mstore_underflow (pc_in : Nat) (top : StackTop)
    ...
    (hunder : top.toNat < 2) :
    runS (Evm.Functions.execute (.MSTORE ()) pc_in top ⟨off, len, msf⟩ g)
        hs ss =
      .ok ((pc_in, top, ⟨off, len, msf⟩, GAS_ZERO), hs)
        { ss with regs := haltRegs ss msg .StackUnderflow } :=
  runS_execute_underflow (.MSTORE ()) 2 0 pc_in top g _ hs ss prof sp
    msg rfl hprof hsp hmsg hfork hunder

open Evm.Functions in
theorem runS_execute_mstore_oog_base (pc_in : Nat) (top : StackTop)
    (off len g : Nat) (msf : EvmMemorySliceFields off len)
    (hs : Evm.HostState) (ss : SeqState)
    (prof : ExecutionProfile) (sp : state_gas_spill) (msg : Evm.Defs.Message)
    (hprof : ss.regs.get? Register.k_execution_profile = some prof)
  ...
  simp only [Evm.Functions.execute,
    show Evm.Functions.opcode_stack_effect (.MSTORE ()) = pure (2, 0)
      from rfl]
  ...
  rw [dif_pos rfl, mstore_dispatch]
  have hbody : runS (Evm.Functions.execute_mstore top ⟨off, len, msf⟩ g)
      hs ss =
      .ok ((top, (⟨off, len, msf⟩ : EvmMemorySlice), GAS_ZERO), hs)
        { ss with regs := haltRegs ss msg .OutOfGas } := by
    simp only [Evm.Functions.execute_mstore]
    refine runS_bind_ok
...
open Evm.Functions in
theorem runS_execute_mstore_oog_exp (pc_in : Nat) (top : StackTop)
    (off len g : Nat) (msf : EvmMemorySliceFields off len)
    (hs : Evm.HostState) (ss : SeqState)
...
open Evm.Functions in
theorem runS_execute_mstore_ok_grow (pc_in : Nat) (top : StackTop)
    (off len g : Nat) (msf : EvmMemorySliceFields off len)
    (hs : Evm.HostState) (ss : SeqState)
    (l : List word) (frest : List (List word)) (x : Nat) (y : word)
    (rest : List word)
...
open Evm.Functions in
theorem runS_execute_mstore_ok_nogrow (pc_in : Nat) (top : StackTop)
    (off len g : Nat) (msf : EvmMemorySliceFields off len)
    (hs : Evm.HostState) (ss : SeqState)
    (l : List word) (frest : List (List word)) (x : Nat) (y : word)
    (rest : List word)
...
open Evm.Functions in
/-- **MSTORE, all reachable outcomes** (success splits on whether the frame
window grows). Requires the memory relation and the MM-6 gas budget. -/
theorem mstore_step_equiv (sRef : Machine) (top : StackTop) (g : Nat)
    ...
    StepResultRel MemPost (runR iMstore sRef)
      (runS (Evm.Functions.execute (.MSTORE ()) pc_in top ⟨off, len, msf⟩ g)
        hs ss) := by
  ...
  | [] =>
    rw [hS] at hpfx htop
    rw [runR_iMstore_underflow_nil sRef hS,
      runS_execute_mstore_underflow pc_in top off len g msf hs ss prof
  ...
  | [x] =>
    rw [hS] at hpfx htop
    rw [runR_iMstore_underflow_one sRef x hS,
      runS_execute_mstore_underflow pc_in top off len g msf hs ss prof
        sRef.evm.stateGasSpilled msg hprof hsp hmsg hfork (by simp at htop; omega)]
    exact StepResultRel.halted ErrorRel.stackUnderflow
      (haltRegs_frame_status ss msg .StackUnderflow)
  | x :: y :: rest =>
    ...
    have hv32 : (toBeBytes32 y).length = 32 := by
      simp [toBeBytes32, natToBytesBE]
    by_cases hg : sRef.evm.gasLeft < mstoreCost sRef.evm.memory.length x
    · rw [runR_iMstore_oog sRef x y rest hS hg]
      by_cases hb : g < G_verylow
      · rw [runS_execute_mstore_oog_base pc_in top off len g msf hs ss prof
      ...
      · push Not at hb
        ...
        have hb2 : g - G_verylow
          ...
          exact key
        rw [runS_execute_mstore_oog_exp pc_in top off len g msf hs ss l
    ...
    · push Not at hg
        ...
        calc ((x :: y :: rest).reverse).take (top.toNat - 2)
          ...
          _ = rest.reverse := by
              rw [List.take_append_of_le_length (by omega), ← hrl,
                List.take_length]
      rw [runR_iMstore_success sRef x y rest hS hg]
      ...
      · rw [runS_execute_mstore_ok_grow pc_in top off len g msf hs ss l
        ...
        have hrelStored := memoryRel_store _ _ off (x + 32) x y hrel'
          (le_refl _)
        rw [hexpandBy]
        ...
        · refine ⟨off, x + 32, {}, rfl, ?_, ?_⟩
          · exact ⟨hrelStored.frame, hrelStored.aligned, hrelStored.bytes,
              hrelStored.tail⟩
          · show MemGasSafe _ (sRef.evm.gasLeft - mstoreCost
              sRef.evm.memory.length x)
            have hsafe2 := memGasSafe_after_expand sRef.evm.memory off len
              (x + 32) sRef.evm.gasLeft
              (mstoreCost sRef.evm.memory.length x) msf haligned hsafe
              (by rw [← hcost]
            ...
            exact hsafe2
      · push Not at hgrow
        rw [runS_execute_mstore_ok_nogrow pc_in top off len g msf hs ss l
          frest x y rest hframe hpfx htop hlim' mfrest hmframe hbase hexp
          hreq hgrow]
        have hrelStored := memoryRel_store sRef.evm.memory hs off len x y
        ...
        · refine ⟨off, len, msf, rfl, ?_, ?_⟩
          ...
          · show MemGasSafe _ (sRef.evm.gasLeft - mstoreCost
            ...
            have hsafe2 := memGasSafe_mono_gas sRef.evm.memory
              (g' := sRef.evm.gasLeft
                - mstoreCost sRef.evm.memory.length x)
