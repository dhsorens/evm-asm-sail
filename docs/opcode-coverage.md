...
| EXTCODECOPY | 0x3c | `iExtcodecopy` | `execute_extcodecopy` | memory+world | unstated (blocked on a byte-level arbitrary-account code relation: current-frame `CodeRel` cannot justify `getAccount`/`getCode` ↔ `k_code_copy`; scalar `ExtcodesizeAgree`/`ExtcodehashAgree` are insufficient) |
