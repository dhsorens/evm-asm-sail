import EvmSpecsVerify.Opcodes.Shapes.Binop

/-!
# LT

Derived through `binop_step_equiv` (`Opcodes/Shapes/Binop.lean`); per-opcode
content is the pure-function lemma and the wf bound. Reachable outcomes:
success / stack underflow / out-of-gas (overflow unreachable for 2-in/1-out).
-/

open private boolPush from EvmAsm.Stateless.SpecRef.InstructionsCore

set_option maxHeartbeats 1000000

namespace EvmSpecsVerify

open EvmAsm.Stateless.SpecRef
open Evm.Defs

theorem alu_lt_eq (a b : Nat) :
    Evm.Functions.alu_lt a b = boolPush (a < b) := by
  show (if (a < b : Bool) = true then ((1 : Int)).toNat else ((0 : Int)).toNat)
      = if (a < b : Bool) = true then 1 else 0
  split <;> rfl

open Evm.Functions in
/-- **LT, all reachable outcomes.** -/
theorem lt_step_equiv (sRef : Machine) (top : StackTop) (g : Nat)
    (hs : Evm.HostState) (ss : SeqState) (mem : EvmMemorySlice) (pc_in : Nat)
    (hrel : StateRel sRef top g hs ss)
    (hpc : pc_in = sRef.evm.pc + 1) :
    StepResultRel (BasePost mem) (runR iLt sRef)
      (runS (Evm.Functions.execute (.LT ()) pc_in top mem g) hs ss) :=
  binop_step_equiv (.LT ()) G_verylow alu_lt iLt GasCosts.OPCODE_LT
    (fun x y => boolPush (x < y)) rfl rfl ⟨rfl, fun _ _ _ _ => rfl⟩
    (fun x y _ _ => alu_lt_eq x y) (fun _ _ _ _ => boolPush_wf _)
    sRef top g hs ss mem pc_in hrel hpc

end EvmSpecsVerify
