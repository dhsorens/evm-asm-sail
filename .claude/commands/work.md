...
3. Take the same default as the coverage UI’s **“Suggested next vertical slice”**
   ...
   - Prefer the first **unstated** follow-up in the coverage UI’s suggested
     queue (`next_slice` / `DATA.nextSlice` — currently MULMOD, then EXP,
     then M2 validators POP / PUSHn / DUPn / MLOAD / JUMPI / SLOAD / RETURN).
   - If that class is exhausted, follow `PROGRESS.md` M1/M2 explicitly.
4. Announce the chosen goal in one sentence before coding
   (“Next slice: MULMOD (`iMulmod` ↔ `execute_mulmod`), full `StepResultRel`”).
