...
| code | `Evm.code : Bytes` (Vm.lean:191) | `frame_code` register (`Code`) + `HostState.codeBytes`/`codeDb` | `CodeRel` | length tie threaded as a hypothesis meanwhile (`codesize_step_equiv`) | unrelated (bytes); length hypothesized (CODESIZE) |
