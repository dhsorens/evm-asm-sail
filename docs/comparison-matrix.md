...
| block env | `BlockEnvironment` (Vm.lean:107) | `k_header` register (`BlockHeader`) + `k_n_headers`, `ancestorHashes` | `BlockEnvRel` | field ties threaded per opcode meanwhile (coinbase/time/number/prevRandao/gasLimit/chainId/baseFee/slotNumber, the `envPush_step_equiv` family); block hashes are BLOCKHASH scope | proven-blockenv-pushers (eight `_step_equiv`s in Opcodes/BlockEnv.lean) |
