...
account access, and push the account balance.
...
The **balance read** is behind the ledgered [`BalanceAgree`](#) hypothesis
(SpecRef's journalled `getAccount` = the kernel's `k_get_balance`, which
...
`k_get_balance` misses through its caches into the keccak-hashed witness
trie. Discharged by the world tranche's account relation. -/
def BalanceAgree (sRef : Machine) (hs : Evm.HostState) (ss : SeqState)
    (x : Nat) : Prop :=
  ∃ (acct : EvmAsm.Stateless.SpecRef.Account) (ts' : TransactionState)
    ...
    (∀ ws, runS (Evm.Functions.k_get_balance
        (Evm.Functions.word_to_address x))
      { hs with warmAddresses := ws } ss = .ok (acct.balance, hostAfter ws)
        ss) ∧
...
theorem runR_accessGasCost_warm (s : Machine) (a : Address)
...
theorem runR_accessGasCost_cold (s : Machine) (a : Address)
...
theorem runR_iBalance_underflow (s : Machine) (hstack : s.evm.stack = []) :
    runR iBalance s = .ok (.error .stackUnderflow, s) := by
  simp only [iBalance]
  exact runR_bind_err (runR_stackPop_nil s hstack)

theorem runR_iBalance_warm_success (s : Machine) (x : U256)
...
theorem runR_iBalance_cold_success (s : Machine) (x : U256)
    (rest : List U256) (acct : EvmAsm.Stateless.SpecRef.Account)
    (ts' : TransactionState)
    (hstack : s.evm.stack = x :: rest)
...
theorem runR_iBalance_warm_oog (s : Machine) (x : U256) (rest : List U256)
    (hstack : s.evm.stack = x :: rest)
    (hwarm : s.evm.accessedAddresses.contains (to_address_masked x) = true)
...
theorem runR_iBalance_cold_oog (s : Machine) (x : U256) (rest : List U256)
    (hstack : s.evm.stack = x :: rest)
    (hcold : s.evm.accessedAddresses.contains (to_address_masked x) = false)
...
open Evm.Functions in
/-- The dispatch equation for BALANCE. -/
theorem balance_dispatch (pc_in : Nat) (top : StackTop)
    (mem : EvmMemorySlice) (g : Nat) :
    Evm.Functions.execute_opcode (.BALANCE ()) pc_in top mem g =
      Evm.Functions.execute_balance top g >>= fun p =>
        pure (pc_in, p.1, mem, p.2) := rfl

open Evm.Functions in
theorem runS_balance_body_ok (top : StackTop) (g : Nat)
    ...
    (bal : word)
    (hostAfter : List (Evm.Defs.address × Nat) → Evm.HostState)
    (hrun : ∀ ws, runS (k_get_balance (Evm.Functions.word_to_address x))
      { hs with warmAddresses := ws } ss = .ok (bal, hostAfter ws) ss)
    (hframes : ∀ ws, (hostAfter ws).stackFrames = hs.stackFrames) :
    runS (Evm.Functions.execute_balance top g) hs ss =
      .ok ((top, g - (if warmb then G_warm_access
        ...
        { hostAfter (wsAfterMark pid (Evm.Functions.word_to_address x) hs)
          with stackFrames := writeListAt l (top.toNat - 1) bal :: frest })
        ss := by
  ...
  simp only [Evm.Functions.execute_balance]
  ...
  refine runS_bind_ok
    (runS_push_word (top - BitVec.ofNat 64 1) bal _ ss l frest
      (by rw [hframes]; exact hframe) (by rw [hret1]; omega)) ?_
  rw [BitVec.sub_add_cancel, hret1]
  exact runS_pure _ _ _

open Evm.Functions in
theorem runS_balance_body_oog (top : StackTop) (g : Nat)
    (hs : Evm.HostState) (ss : SeqState)
    (l : List word) (frest : List (List word)) (x : word) (rest : List word)
    (hframe : hs.stackFrames = l :: frest)
    (hpfx : l.take top.toNat = (x :: rest).reverse)
...
open Evm.Functions in
theorem runS_execute_balance_ok (pc_in : Nat) (top : StackTop) (g : Nat)
  ...
  rw [dif_pos rfl, balance_dispatch]
  refine runS_bind_ok
    (runS_balance_body_ok top g hs ss l frest x rest hframe hpfx htop prof
      hprof hfork pid hpid warmb hwarmb hgas bal hostAfter hrun hframes) ?_
  exact runS_pure _ _ _

open Evm.Functions in
theorem runS_execute_balance_oog (pc_in : Nat) (top : StackTop) (g : Nat)
    (mem : EvmMemorySlice) (hs : Evm.HostState) (ss : SeqState)
    (l : List word) (frest : List (List word)) (x : word) (rest : List word)
    (hframe : hs.stackFrames = l :: frest)
    (hpfx : l.take top.toNat = (x :: rest).reverse)
  ...
  rw [dif_pos rfl, balance_dispatch]
  refine runS_bind_ok
    (runS_balance_body_oog top g hs ss l frest x rest hframe hpfx htop prof
      sp msg hprof hsp hmsg hfork pid hpid warmb hwarmb hgas) ?_
  exact runS_pure _ _ _

open Evm.Functions in
theorem runS_execute_balance_underflow (pc_in : Nat) (top : StackTop)
...
/-- Success post-relation for BALANCE: the ALU-slice relation plus the
...
hypotheses); the balance read is supplied by the ledgered `BalanceAgree`
hypothesis. -/
theorem balance_step_equiv (sRef : Machine) (top : StackTop) (g : Nat)
    ...
    (hagree : ∀ (x : U256) (rest : List U256),
      sRef.evm.stack = x :: rest → BalanceAgree sRef hs ss x) :
    StepResultRel (BalancePost pid mem) (runR iBalance sRef)
      (runS (Evm.Functions.execute (.BALANCE ()) pc_in top mem g) hs ss) := by
  ...
  | [] =>
    rw [hS] at hpfx htop
    rw [runR_iBalance_underflow sRef hS,
      runS_execute_balance_underflow pc_in top g mem hs ss prof
  ...
  | x :: rest =>
    ...
    obtain ⟨acct, ts', hostAfter, hwfb, hspec, hrun, hframes, hslots,
      hepoch⟩ := hagree x rest hS
    ...
    | true =>
      ...
      · rw [runR_iBalance_warm_oog sRef x rest hS hwc hg,
          runS_execute_balance_oog pc_in top g mem hs ss l frest x rest
      ...
      · push Not at hg
        rw [runR_iBalance_warm_success sRef x rest acct ts' hS hwc hg hspec
            (by rw [hS]; exact hlim),
          runS_execute_balance_ok pc_in top g mem hs ss l frest x rest
            hframe hpfx htop hlim' prof hprof hfork pid (hpid aV) true hwb
            (by rw [if_pos rfl, hlive]; exact hg) acct.balance hostAfter
            hrun hframes]
        rw [if_pos rfl]
    ...
    | false =>
      ...
      · rw [runR_iBalance_cold_oog sRef x rest hS hwc hg,
          runS_execute_balance_oog pc_in top g mem hs ss l frest x rest
            hframe hpfx htop hlim' prof sRef.evm.stateGasSpilled msg hprof
        ...
        exact StepResultRel.halted ErrorRel.outOfGas
          (haltRegs_frame_status ss msg .OutOfGas)
      · push Not at hg
        rw [runR_iBalance_cold_success sRef x rest acct ts' hS hwc hg hspec
            (by rw [hS]; exact hlim),
          runS_execute_balance_ok pc_in top g mem hs ss l frest x rest
            hframe hpfx htop hlim' prof hprof hfork pid (hpid aV) false hwb
            (by rw [if_neg (by simp), hlive]; exact hg) acct.balance
