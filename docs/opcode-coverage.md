...
| LOG n | 0xa0–0xa4 | `iLogN` | `execute_log` | memory+logs | unstated (largest remaining slice: needs a `LogRel` plus a `Shapes/Log.lean` for the five-arity fan-out — see `PROGRESS.md`. MM-11 records the static-guard order divergence found here; the LOG gas constants are verified by inspection under MM-2) |
